...
import { Bell, BookOpen, CalendarDays, ChartNoAxesCombined, CheckCircle2, CircleDollarSign, ClipboardList, GraduationCap, Headphones, Home, Library, LogOut, Medal, Radio, Settings, UserRound, Video } from "lucide-react";
...
export function PortalSidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const { studentData, logout } = usePortalAuth();
  const isStaff = isStaffRole(studentData?.role);
  const items = isStaff ? teacherNav : studentNav;
  return (
    <aside className="flex h-full w-72 flex-col border-r border-slate-200 bg-white">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4">
        {items.map(([label, href, Icon]) => {
          ...
          return (
            <Link key={href} href={href} onClick={onNavigate} className={`flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold transition ${active ? "bg-violet-50 text-[#6C3CE9]" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>
              <Icon size={18} />{label}
            </Link>
          );
        })}
      ...
      <div className="border-t border-slate-100 p-4">
        <p className="truncate px-3 text-xs font-semibold text-slate-700">{studentData?.fullName}</p>
        <p className="truncate px-3 text-[11px] uppercase tracking-wide text-slate-400">{isStaff ? studentData?.role : "Student"}</p>
